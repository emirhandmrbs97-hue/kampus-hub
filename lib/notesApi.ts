...
export async function uploadNoteFile(fileUri: string, filename: string, contentType: string) {
  ...
  const { error: uploadError } = await supabase.storage.from(NOTES_BUCKET).upload(path, blob, { contentType: 'application/pdf' });
...
export async function createNoteRecord(payload: {
  ...
  const { data, error } = await supabase.from('notes').insert([payload]).select().maybeSingle();
...
export async function fetchNotes({
  ...
  let query = supabase.from('notes').select('id, author, course, section, class, semester, title, description, filename, file_path, downloads, views, created_at, author:profiles(id, full_name, avatar_url, username)').order('created_at', { ascending: false }).range(offset, offset + limit - 1);
...
export async function deleteNote(noteId: string) {
  ...
  const { error: delStorageErr } = await supabase.storage.from(NOTES_BUCKET).remove([note.file_path]);
  if (delStorageErr) {
    // Not fatal, but warn
    console.warn('Failed to remove file from storage', delStorageErr.message);
  }
